...
test.describe('Login functionality', () => {
  ...
  test('shows an error for valid username with wrong password', async ({ loginPage }) => {
    await loginPage.login(users.validUsernameWrongPassword);
    await loginPage.expectError(loginErrors.invalidCredentials);
  });

  test('shows an error for invalid username with valid password', async ({ loginPage }) => {
  ...
  test('shows an error when both username and password are missing', async ({ loginPage }) => {
    await loginPage.login(users.emptyCredentials);
    await loginPage.expectError(loginErrors.usernameRequired);
  });

  test('shows an error when username casing does not match', async ({ loginPage }) => {
    await loginPage.login(users.uppercaseUsername);
    await loginPage.expectError(loginErrors.invalidCredentials);
  });

  test('shows an error when username contains extra whitespace', async ({ loginPage }) => {
